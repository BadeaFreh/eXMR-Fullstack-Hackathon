...
$(document).on('click', '.submit-btn', async function () {
    ...
    const newReview = await model.postNewReview(
        songName,
        singerName,
        youtubeLink,
        selectedCategory,
...
function updateReactions(reviewId, reactionClicked, prevReaction) {
    if (
        prevReaction === reactionClicked &&
        reviewsReactedTo[reviewId].reactionClicked === 'on'
    ) {
        let selectedReactionCounter =
            parseInt(
                $(`body #${reviewId} .${reactionClicked}-btn`).text().split[1]
            ) - 1
        $(`.${reactionClicked}-btn`).text(
            `${emojees.reactionClicked} ${selectedReactionCounter}`
        )
